...
export default function ToolsPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-24 space-y-8 relative">
      ...
      {/* Header */}
      <header className="pb-6 border-b border-line relative z-10">
        ...
        <p className="text-sm text-ink/30 mt-2">Analyze contracts, generate clauses, and get legal insights.</p>
      </header>
      ...
      <div className="grid sm:grid-cols-3 gap-4 relative z-10">
        {[
          ...
          <div key={tool.label} className={cn(
            ...
            <div>
              <p className="text-[11px] font-black text-ink uppercase tracking-widest">{tool.label}</p>
              <p className="text-[10px] text-ink/30 mt-1">{tool.desc}</p>
            </div>
            {!tool.active && <span className="text-[8px] font-black text-ink/20 uppercase tracking-widest">Coming Soon</span>}
          </div>
        ))}
      </div>
      ...
      <div className="relative z-10 space-y-5">
        <div className="flex items-center gap-3 pb-4 border-b border-line">
          ...
          <div>
            <h2 className="text-sm font-black text-ink uppercase tracking-tight">Contract Analyzer</h2>
            <p className="text-[10px] text-ink/30 uppercase tracking-widest">AI-powered risk assessment</p>
          </div>
        </div>
                ...
                <div className="flex-1 space-y-2.5 w-full">
                  {Object.entries(analysis.scores).map(([key, val]) => (
                    <div key={key} className="flex items-center gap-3">
                      <span className="text-[9px] font-black text-ink/30 uppercase tracking-widest w-24 flex-shrink-0">{key}</span>
              ...
              <div className="p-5 rounded-[3px] bg-ink/[0.02] border border-line">
                ...
                {analysis.contractType && (
                  <div className="flex items-center gap-2 mt-3">
                    <span className="text-[9px] font-black text-ink/20 uppercase tracking-widest">Detected:</span>
                    ...
                    {analysis.parties?.map(p => (
                      <span key={p} className="text-[10px] font-black text-ink/40 uppercase tracking-widest px-2 py-0.5 rounded-full bg-ink/[0.04] border border-line">{p}</span>
                    ))}
                  </div>
                )}
              </div>
                ...
                <div className="rounded-[3px] border border-line overflow-hidden">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-black text-ink uppercase tracking-widest">Red Flags</span>
                    ...
                    {expanded === 'flags' ? <CaretUp size={14} className="text-ink/30" weight="bold" /> : <CaretDown size={14} className="text-ink/30" weight="bold" />}
                  </button>
                  <AnimatePresence>
                    {expanded === 'flags' && (
                            ...
                            return (
                              <div key={i} className={cn('p-4 rounded-[3px] border flex gap-3', cfg.bg, cfg.border)}>
                                ...
                                <div>
                                  ...
                                  <p className="text-[10px] text-ink/40 mt-0.5 leading-relaxed">{flag.description}</p>
                                </div>
                              </div>
                            );
                          })}
                ...
                <div className="rounded-[3px] border border-line overflow-hidden">
                  <button onClick={() => toggle('missing')} className="w-full flex items-center justify-between p-4 hover:bg-ink/[0.02] transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-black text-ink uppercase tracking-widest">Missing Clauses</span>
                    ...
                    {expanded === 'missing' ? <CaretUp size={14} className="text-ink/30" weight="bold" /> : <CaretDown size={14} className="text-ink/30" weight="bold" />}
                  </button>
                  <AnimatePresence>
                    {expanded === 'missing' && (
                      <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                ...
                <div className="rounded-[3px] border border-line overflow-hidden">
                  <button onClick={() => toggle('strengths')} className="w-full flex items-center justify-between p-4 hover:bg-ink/[0.02] transition-colors">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-[11px] font-black text-ink uppercase tracking-widest">Strengths</span>
                    ...
                    {expanded === 'strengths' ? <CaretUp size={14} className="text-ink/30" weight="bold" /> : <CaretDown size={14} className="text-ink/30" weight="bold" />}
                  </button>
                  <AnimatePresence>
                    {expanded === 'strengths' && (
                      <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                  ...
                  <div className="space-y-2">
                    {analysis.recommendations.map((rec, i) => (
                      <div key={i} className="flex items-start gap-3">
                        ...
                        <p className="text-[11px] text-ink/60 leading-relaxed">{rec}</p>
                      </div>
                    ))}
                  </div>
                </div>
        ...
        {!analysis && !loading && (
          <div className="py-12 flex flex-col items-center gap-4 text-center">
            ...
            <p className="text-[10px] text-ink/10 max-w-xs">The AI will analyze risk, missing clauses, enforceability, and give you improvement recommendations.</p>
          </div>
        )}
      </div>
    </div>
